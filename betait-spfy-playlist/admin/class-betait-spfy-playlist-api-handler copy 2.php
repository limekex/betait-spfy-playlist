...
class Betait_Spfy_Playlist_API_Handler {
    ...
    public function search_tracks( $query, $access_token = null ) {
        ...
        if ( ! $this->credentials_are_set() && ! $access_token ) {
            $this->log_debug( 'Spotify API credentials are not set, and no access token was provided.' );
            return array(
                'success' => false,
                'data'    => array(
                    'message' => __( 'Spotify API credentials are not set.', 'betait-spfy-playlist' ),
                ),
            );
        }
    
        // Retrieve an access token if not explicitly provided.
        if ( ! $access_token ) {
            $token = $this->get_access_token();
            if ( is_array( $token ) && isset( $token['error'] ) ) {
                $this->log_debug( 'Error retrieving access token: ' . $token['error'] );
                return array(
                    'success' => false,
                    'data'    => array(
                        'message' => $token['error'],
        ...
        $this->log_debug( 'Using access token for search: ' . substr( $access_token, 0, 10 ) . '...' );
    
        // Build the API request URL and arguments.
        ...
        $this->log_debug( 'Spotify API search request URL: ' . $url );
        ...
        // Handle successful responses with track data.

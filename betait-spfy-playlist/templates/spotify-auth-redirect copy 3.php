...
if ( ! isset( $_GET['access_token'] ) ) {
    log_debug( 'No access_token found. Redirecting to home.' );
    wp_redirect( home_url() );
    exit;
}

// Log the entire query string for debugging purposes.
log_debug( 'Full query string: ' . print_r( $_GET, true ) );
...
if ( $current_user_id ) {
    ...
    log_debug( 'Access token saved to user meta for user ID: ' . $current_user_id );
} else {
    log_debug( 'No user logged in. Access token not saved to user meta.' );
}
